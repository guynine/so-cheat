...
template <typename T> T writem(unsigned long long address) {
	T buffer{};
	writem(address, &buffer, sizeof(T));
	return buffer;
}
template <typename T> T writem(void* address) {
	T buffer{};
	writem((unsigned long long)address, &buffer, sizeof(T));
	return buffer;
}